...
const WorkOrderDetail = ({ user }: any) => {
  ...
  const [activeTab, setActiveTab] = useState<"DETAILS" | "TASKS" | "TIME" | "PARTS" | "FILES">("DETAILS");
  ...
  const [timeEntries, setTimeEntries] = useState<{ id: number; worker: string; duration: number; date: string }[]>([]);
  ...
  useEffect(() => {
    ...
    if (user?.organizationId) {
      fetch(`http://${API_URL}/api/users/${user.organizationId}`).then((res) => res.json()).then(setOrgUsers);
      fetch(`http://${API_URL}/api/assets?orgId=${user.organizationId}`).then((res) => res.json()).then(setOrgAssets);
      fetch(`http://${API_URL}/api/locations?orgId=${user.organizationId}`).then((res) => res.json()).then(setOrgLocations);
      fetch(`http://${API_URL}/api/inventory?orgId=${user.organizationId}`).then((res) => res.json()).then(setOrgParts);
      fetch(`http://${API_URL}/api/teams?orgId=${user.organizationId}`).then((res) => res.json()).then(setOrgTeams);
    }
  ...
  const saveAssignees = async (newAssignees: any[], logMsg: string) => {
    ...
    const additional = newAssignees.length > 1 ? newAssignees.slice(1).map(u => u.email).join(',') : null;
  ...
  const selectMentionUser = (userObj: any) => {
    ...
    if (lastAtIndex !== -1) {
      const updatedText = newComment.slice(0, lastAtIndex) + `@${userObj.firstName} ${userObj.lastName} `;
  ...
  const handleAddPart = async () => {
    ...
    if (partObj) {
      setParts([...parts, { id: Date.now(), partId: partObj.id, name: partObj.name, qty: selectedPartQty }]);
  ...
  const handleAddTime = async () => {
    ...
    setTimeEntries([...timeEntries, { id: Date.now(), worker: `${user.firstName} ${user.lastName}`, duration: parseFloat(newTimeDuration), date: new Date().toLocaleString() }]);
  ...
  const renderFormattedComment = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('@')) {
        ...
        const taggedUser = orgUsers.find(u => 
        ...
        return (
          <span key={i} className="text-blue-600 bg-blue-50 font-black px-1.5 py-0.5 rounded-md inline-block my-0.5 border border-blue-100 shadow-2xs">
            {part}
          </span>
        );
      }
  ...
  if (loading) return <div className="p-8 text-gray-500 flex items-center justify-center h-full">Loading ticket...</div>;
    ...
    <div className="flex flex-col h-full flex-1 bg-gray-50 font-sans overflow-hidden">
      ...
      <div className="bg-white border-b border-gray-200 px-4 md:px-6 py-4 flex items-center justify-between shrink-0 shadow-sm relative z-10">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/workspace/workorders")} className="p-1.5 hover:bg-gray-100 rounded-md text-gray-500 transition-colors">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className={`flex items-center px-3 py-1.5 md:px-4 md:py-2 rounded-lg font-bold text-xs md:text-sm cursor-pointer outline-none border transition-colors ${
              isCompleted ? "bg-green-50 text-green-700 border-green-200" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-50"
            }`}
          >
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className={`flex-1 flex flex-col bg-white overflow-hidden border-r border-gray-200 transition-all ${isActivityOpen ? "hidden md:flex" : "flex"}`}>
            ...
            {isEditingHeader ? (
              <div className="space-y-4 max-w-4xl">
                ...
                <div className="flex gap-3">
                  <button onClick={handleSaveHeader} className="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-bold shadow-sm hover:bg-blue-700 transition-colors">
                    Save Changes
                  </button>
                  <button onClick={() => setIsEditingHeader(false)} className="bg-gray-100 text-gray-600 px-5 py-2 rounded-lg text-sm font-bold shadow-sm hover:bg-gray-200 transition-colors">
                    Cancel
                  </button>
                </div>
              </div>
              ...
              <div className="flex justify-between items-start max-w-4xl">
                ...
                <div className="flex gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={startEditingHeader} className="p-2 text-gray-500 hover:text-blue-600 bg-gray-50 hover:bg-blue-50 border border-gray-200 rounded-md transition-colors shadow-sm">
                    <Pencil className="w-4 h-4" />
                  </button>
                  <button onClick={handleDeleteWO} className="p-2 text-gray-500 hover:text-red-600 bg-gray-50 hover:bg-red-50 border border-gray-200 rounded-md transition-colors shadow-sm">
                ...
                <div className="border border-gray-200 rounded-2xl overflow-hidden divide-y divide-gray-100 shadow-sm">
                  <EditableRow label="LOCATION">
                    <select
                      ...
                      onChange={(e) => handleInlineUpdate("locationName", e.target.value, `changed location to ${e.target.options[e.target.selectedIndex].text}`)}
                  ...
                  <EditableRow label="ASSET">
                    <select
                      value={wo.assetId || ""}
                      onChange={(e) => handleInlineUpdate("assetId", e.target.value || null, `changed asset to ${e.target.options[e.target.selectedIndex].text}`)}
                      className="w-full bg-transparent text-xs md:text-sm font-medium text-blue-600 hover:text-blue-800 cursor-pointer outline-none"
                    >
                    ...
                    <div className="relative w-full" ref={assigneesRef}>
                      ...
                      {isAssigneeDropdownOpen && (
                        <div className="absolute top-full left-0 mt-2 w-full bg-white border border-gray-200 shadow-xl rounded-xl z-50 py-2 max-h-48 overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
                          ...
                          {orgUsers.filter((u) => !selectedAssignees.some((a) => a.id === u.id)).length === 0 && (
                            <div className="px-4 py-2 text-xs text-gray-400 italic">No additional users available</div>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <EditableRow label="OPERATIONAL TEAM">
                    <select
                      value={wo.teamId || ""}
                      onChange={(e) => handleInlineUpdate("teamId", e.target.value || null, `changed operational team to ${e.target.options[e.target.selectedIndex].text}`)}
                      className="w-full bg-transparent text-xs md:text-sm font-medium text-blue-600 hover:text-blue-800 cursor-pointer outline-none"
                    >
                  ...
                  <EditableRow label="CATEGORY">
                    <select
                      value={wo.category || ""}
                      onChange={(e) => handleInlineUpdate("category", e.target.value || null, `changed category to ${e.target.value.replace(/_/g, " ")}`)}
                  ...
                  <EditableRow label="PRIORITY">
                    <select
                      ...
                      onChange={(e) => handleInlineUpdate("priority", e.target.value, `changed priority to ${e.target.value}`)}
                      className="w-full bg-transparent text-xs md:text-sm font-medium text-gray-900 cursor-pointer outline-none"
                    >
                  ...
                  <EditableRow label="EST. DURATION">
                    <input
                      ...
                      onChange={(e) => handleInlineUpdate("estimatedHours", parseFloat(e.target.value) || null, `changed estimated duration to ${e.target.value} hours`)}
                      placeholder="0.0"
                  ...
                  <EditableRow label="DUE DATE">
                    <input
                      ...
                      onChange={(e) => handleInlineUpdate("dueDate", e.target.value ? new Date(e.target.value).toISOString() : null, "changed due date")}
            ...
            {activeTab === "TASKS" && (
              <div className="max-w-3xl">
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6 flex flex-col sm:flex-row gap-3">
                  ...
                  />
                  <button onClick={handleAddTask} className="w-full sm:w-auto bg-blue-600 text-white px-6 py-2.5 rounded-lg text-sm font-bold active:scale-95 transition-all">
                    Add
                  </button>
                </div>
                <div className="space-y-3">
                  {tasks.map((task) => (
                    <div key={task.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-xl">
                      <div className="flex items-center gap-3">
                        <input
                          type="checkbox"
                      ...
                      <button onClick={() => removeTask(task.id, task.text)} className="text-gray-400 hover:text-red-600">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
              ...
              <div className="max-w-3xl">
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6 flex flex-col sm:flex-row gap-3">
                  ...
                  <div className="flex gap-3 w-full sm:w-auto">
                    ...
                    />
                    <button onClick={handleAddPart} className="flex-1 sm:flex-none bg-blue-600 text-white px-6 py-2.5 rounded-lg text-sm font-bold active:scale-95 transition-all">
                      Add
                    </button>
                  </div>
                </div>
                <div className="space-y-3">
                  {parts.map((p) => (
                    <div key={p.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-xl">
                      <span className="text-sm font-bold">{p.name} (Qty: {p.qty})</span>
                      <button onClick={() => removePart(p.id, p.name)} className="text-gray-400 hover:text-red-600">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
              ...
              <div className="max-w-3xl">
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6 flex flex-col sm:flex-row gap-3">
                  <input
                    ...
                    className="w-full sm:flex-1 bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-sm outline-none"
                  />
                  <button onClick={handleAddTime} className="w-full sm:w-auto bg-blue-600 text-white px-6 py-2.5 rounded-lg text-sm font-bold active:scale-95 transition-all">
                    Log Time
                  </button>
                </div>
                <div className="space-y-3">
                  {timeEntries.map((t) => (
                    <div key={t.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-xl">
                      <div>
              ...
              <div className="max-w-3xl">
                ...
                <div className="space-y-3">
                  {(wo.documents || []).map((doc: any) => (
                    <div key={doc.id} className="flex items-center p-3 bg-white border border-gray-200 rounded-xl">
                      <FileText className="w-6 h-6 text-blue-600 mr-3" />
                      <a href={`http://${API_URL}${doc.fileUrl}`} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-blue-600 hover:underline">
                        {doc.fileName}
                      </a>
                    </div>
                  ))}
        ...
        <div className={`flex flex-col bg-gray-50 shrink-0 border-l border-gray-200 transition-all duration-300 ease-in-out h-full overflow-hidden ${isActivityOpen ? "w-full md:w-[380px]" : "w-0 border-l-0"}`}>
          ...
          <div className="px-4 py-4 border-b border-gray-200 bg-white shrink-0 flex items-center justify-between h-14 w-full md:w-[380px]">
            <div className="flex items-center">
              <button onClick={() => setIsActivityOpen(false)} className="md:hidden p-1 hover:bg-gray-100 rounded-md text-gray-400 hover:text-gray-900 transition-colors mr-2">
                <ArrowLeft className="w-5 h-5" />
              </button>
              <h3 className="text-sm font-black tracking-wide text-gray-900">Comments & Activity</h3>
            </div>
            <button onClick={() => setIsActivityOpen(false)} className="text-gray-400 hover:text-gray-700 p-1 rounded-full hover:bg-gray-100 transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>

          {/* Activity Scroll Area */}
          <div ref={activityScrollRef} className="flex-1 overflow-y-auto p-4 md:p-6 space-y-5 w-full md:w-[380px] bg-gray-50 pb-6">
            {activities.length === 0 ? (
                ...
                return (
                  <div key={`${item.type}-${item.id}-${idx}`} className="flex gap-3 group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`text-xs md:text-sm whitespace-pre-wrap break-words leading-relaxed ${isSystemLog ? "text-gray-500 italic" : "text-gray-800 bg-white border border-gray-200 px-3.5 py-2.5 rounded-2xl rounded-tl-sm shadow-sm"}`}>
            ...
            {showMentions && (
              <div className="absolute bottom-full left-3 right-3 mb-2 bg-white border border-gray-200 shadow-2xl rounded-2xl overflow-hidden z-50 max-h-48 overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
                ...
                {filteredMentionUsers.length > 0 ? (
                  ...
                  <div className="px-4 py-3 text-xs text-gray-400 italic">No users found matching "{mentionQuery}"</div>
                )}
              </div>
            )}
